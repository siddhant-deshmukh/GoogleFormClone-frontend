...
import { EdtiFormAction, IAllFormQuestions, IEditFormState, IQuestionForm } from '../types'
...
function FormEditor(
  { formId, editFormState, selectQuestionRef, dispatch }: {
    formId: string | undefined,
    editFormState: IEditFormState,
    dispatch: React.Dispatch<EdtiFormAction>,
  ...
  useEffect(() => {
    if (editFormState.queListState.length === 0) return;
    const queSeq_ = editFormState.queListState.map((ele) => { return ele.id })
